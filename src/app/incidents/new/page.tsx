  ...
  return (
    <div className="page-root animate-fade-up" style={{ maxWidth: 920, margin: '0 auto' }}>
      ...
      <InteractionRailLayout
        ...
        rail={(
          <>
            <DataCard title="新建边界" subtitle="创建页主区只保留初报表单和提交流程。" badge={<Tag variant="warning">Boundary</Tag>}>
              <div style={{ display: 'grid', gap: 10 }}>
                <div className="page-help-card-item">提交后先进入待分派，由值班主管确认责任人。</div>
                <div className="page-help-card-item">严重程度和时间字段必须先校验，不在主区混排说明型内容。</div>
                <div className="page-help-card-item">完整页面定位与处置口径迁移到帮助页。</div>
              </div>
            </DataCard>

            <PageHelpCard
              title="页面帮助"
              subtitle="完整事件创建说明迁移到显式帮助页"
              summary="事件新建页现在只保留事故初报流程和表单字段，页面解释与处置顺序统一后置。"
              items={[
                '先录入事故初报，再提交进入待分派。',
                '待分派前不直接进入结案或复盘。',
                '若需要完整说明，进入事件帮助页查看。',
