  ...
  const buildAiHref = (focus: string, target: 'inference' | 'rules' | 'logs' = 'inference') => buildAiAssistantHref({
    ...
    return (
      <div className="page-root animate-fade-up">
        ...
        <DataCard
          title="联动说明"
          subtitle="任务台账不再只依赖个案受理，而是同时消费个案评定与认定模板两条 workflow。"
        ...
        >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 12 }}>
            <div style={{ padding: 14, borderRadius: 'var(--radius-md)', background: 'var(--color-bg)' }}>
              ...
              <div style={{ marginTop: 6, fontSize: 12.5, lineHeight: 1.6, color: 'var(--color-muted)' }}>个案评定页确认认定后生成首评任务；模板页复核通过后生成复评、抽检和整改派生任务。</div>
            </div>
            <div style={{ padding: 14, borderRadius: 'var(--radius-md)', background: 'var(--color-bg)' }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--color-text)' }}>执行口径</div>
              <div style={{ marginTop: 6, fontSize: 12.5, lineHeight: 1.6, color: 'var(--color-muted)' }}>模板派生任务完成后会反写归档状态，避免模板页与任务中心出现双向不一致。</div>
            </div>
            <div style={{ padding: 14, borderRadius: 'var(--radius-md)', background: 'var(--color-bg)' }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--color-text)' }}>待处理提醒</div>
              <div style={{ marginTop: 6, fontSize: 12.5, lineHeight: 1.6, color: 'var(--color-muted)' }}>当前还有 {stats.pendingPlanReviews} 条认定模板待复核，未复核前不会进入现场评定闭环。</div>
            </div>
            <div style={{ padding: 14, borderRadius: 'var(--radius-md)', background: 'var(--color-bg)' }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--color-text)' }}>回滚方式</div>
              <div style={{ marginTop: 6, fontSize: 12.5, lineHeight: 1.6, color: 'var(--color-muted)' }}>若需回退，可先移除模板派生来源，仅保留个案评定派生任务，不影响现有 assessment store。</div>
            </div>
          </div>
        ...
        <DataCard title="评定执行任务" subtitle="统一展示首评、复评复核、抽检回访和整改任务，保留执行备注和归档口径。">
